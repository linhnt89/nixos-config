...
- The desktop's Home Manager config imports the **desktop role** of the `nixdev-config` flake (`nixdev-config.homeManagerModules.desktop` in `flake.nix`): it owns the portable shell/dev layer (zsh/starship/fzf/bat/eza enablement, direnv, delta/git structure, common packages incl. Python/Node, `gh` — never `glab`). Local `home/modules/{shell,git,dev}.nix` are personal adapters; keep MetaCube identity, SSH hosts, appearance/apps/services/Waybar/Mango/Noctalia, Pi seed, and Firstmate/treehouse/herdr tooling local. Public input: fetching needs no GitHub token (never commit a credential, and never substitute a `path:` input); updates land via Dependabot or `nix flake lock --update-input nixdev-config`. `scripts/check.sh` now also evaluates the imported HM user config. See `docs/nixdev-config-integration.md`.
